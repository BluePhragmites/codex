...
## gnb_c Documentation Roles
The authoritative `gnb_c` documentation is split by purpose. Keep those boundaries clean instead of duplicating the same content across files.

- `gnb_c/README.md`
  - what the project is
  - build dependencies
  - build commands
  - main run commands and output locations
- `gnb_c/implementation_plan.md`
  - current implemented features
  - known limitations
  - future roadmap and next priorities
- `gnb_c/feature_test_guide.md`
  - test layers
  - feature-to-test mapping
  - runnable validation commands
  - manual Open5GS verification steps
- `gnb_c/architecture.md`
  - architecture overview
  - module boundaries
  - data and control flow
  - recommended code-reading order

When working on `gnb_c/`, preserve that separation of concerns. Do not turn `README.md` back into a combined architecture, roadmap, and test notebook.

## gnb_c Development Plan
The active `gnb_c` status and roadmap are tracked in [gnb_c/implementation_plan.md](gnb_c/implementation_plan.md). Use that file as the source of truth for current features, known limitations, and next development priorities.
...
- `gnb_c/README.md`: build, dependencies, and user-visible run entrypoints
- `gnb_c/architecture.md`: architecture, module boundary, workflow, and learning-path changes
- `gnb_c/implementation_plan.md`: current features, known limitations, and roadmap changes
- `gnb_c/feature_test_guide.md`: test matrix, validation commands, and evidence for implemented capabilities
